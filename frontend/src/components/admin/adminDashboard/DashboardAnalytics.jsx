...
export function DashboardAnalytics({
  ...
  return (
    ...
    >
      ...
      <div className="rounded-3xl bg-surface-container-lowest p-4 tablet:p-5 dark:bg-surface-container-low border border-outline-variant/20 shadow-xs flex flex-col">
        ...
        <div className="flex-1 space-y-2.5">
          {dayBreakdown.map((item) => (
            <div key={item.day} className="space-y-1">
              ...
              <div className="h-2 w-full rounded-full bg-surface-container-high/60 overflow-hidden shadow-inner">
                <div
        ...
        <div className="flex-1 space-y-2">
          {classTypeBreakdown.length === 0 ? (
            ...
            classTypeBreakdown.map((item) => (
              <div
                key={item.key}
                className="flex items-center justify-between gap-2 rounded-2xl border border-outline-variant/20 bg-surface-container-low/40 p-2.5 tablet:p-3 dark:bg-surface-container-high/30 shadow-2xs"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span
                    className={`inline-flex shrink-0 items-center rounded-full px-2.5 py-0.5 text-[10px] font-bold border shadow-2xs ${item.meta.tone}`}
                  >
                    {item.key}
                  </span>
                ...
                <span className="shrink-0 text-body-sm font-extrabold text-on-surface">
                  {item.count}
                </span>
              </div>
            ))
            ...
            <div className="space-y-2">
              {prodiBreakdown.slice(0, 6).map((p) => (
                ...
                >
                  ...
                  <span className="shrink-0 text-body-sm font-extrabold text-primary">
                    {p.sessionCount}
                  </span>
                </div>
              ))}
