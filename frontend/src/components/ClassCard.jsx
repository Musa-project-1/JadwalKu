...
import { getClassType, TONE_TEXT_CLASSES, TONE_DOT_CLASSES, TONE_BG_CLASSES } from '../lib/classTypes'

/**
 * Kartu kelas — gaya "JadwalKu Expressive": 24px radius, 4px status bar di
 * kiri (kembali sesuai referensi v2), latar tonal 8% tanpa border.
 */
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-xs">
        <h3 className={`truncate text-title-md ${text}`}>
          {course?.namaMK ?? entry.kodeMK}
        </h3>
        {conflicted && <Icon name="warning" size={18} className="shrink-0 text-error" />}
      </div>
      <p className="mt-1 flex items-center gap-1.5 text-body-sm opacity-90">
